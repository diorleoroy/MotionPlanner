...
public class bfs_new3 : MonoBehaviour
{
	...
	bool[,,] visited_point = new bool[128,128,36];
...
//				Debug.Log (k);
				...
				for (int i = 0; i < 4; i++) {
					...
					if (visited_point [(int)robot_position.x,(int)robot_position.y,(int)robot_position.z/10] == true) {
					...
					if (already_visited == false && collis == false && now_potential < 508) {
						...
						visited_point [(int)tmp_root_data.root_point.x, (int)tmp_root_data.root_point.y,(int) tmp_root_data.arc/10] = true;
				...
				for (int i = 1; i <= 1; i++) {
					...
					gameObject.transform.Rotate (new Vector3 (0, 0, 10 * i));
					...
					if (already_visited == false && collis == false && rotate_now_potential < 508) {
						...
						visited_point [(int)rotate_tmp_root_data.root_point.x, (int)rotate_tmp_root_data.root_point.y, (int)rotate_tmp_root_data.arc/10] = true;
	...
	bool collis_bomb()
	{
		...
		for (int i = 0; i < sub_robot_msg.Length; i++) {
			bomb_tmp = sub_robot_msg [i].GetComponent<collision_detection_2> ().detection_bomb ();
			if (bomb_tmp == true) {
				return true;
			}
		}
